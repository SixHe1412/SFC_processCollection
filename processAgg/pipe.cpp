...
int main(int argc, char* argv[])
{
	...
	{
		redisClusterAppendCommand(cc, "zrangebyscore %s-level %s %s", lvl, vec_res2[i].str().c_str(), vec_res2[i + 1].str().c_str()); ///r1,r2
	...
	for (int i = 0; i < count; i++)
	{
		...
		for (int j = 0; j < reply->elements; j++)
		{
			...
			////decode
			sfc_bigint val("1234567890"); ///---*give the sfc value**-----
