...
retry:
    ...
    if(node == NULL)
    {
        __redisClusterSetError(cc, REDIS_ERR_OTHER, "node get by table error1");
        return NULL;
    }
    ...
    struct cmd *command, hilist *commands)
{
    ...
    if (command->type == CMD_REQ_REDIS_MGET) {
        ...
        str_len = strlen(REDIS_STATUS_OK);
    ...
    redisClusterAsyncContext *acc, int slot_num)
{
    ...
    if(node == NULL)
    {
        __redisClusterAsyncSetError(acc, 
            REDIS_ERR_OTHER, "node get by table error2");
        return NULL;
    }
...
int redisClusterAsyncFormattedCommand(redisClusterAsyncContext *acc, 
    ...
    {
        __redisClusterAsyncSetError(acc, 
            REDIS_ERR_OTHER, "node get by table error3");
        goto error;
    }
