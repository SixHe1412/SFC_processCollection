...
//////////////////////////////////////////////////////////////////////////
int write_(int fd, char* buf, size_t len)
{
#ifdef _WIN32
...
int read_(int fd, char* buf, size_t len)
{
#ifdef _WIN32
...
int set_blockmode(int fd, int block)
{//block=1×èÈû£¬0·Ç×èÈû
#ifdef _WIN32
	unsigned int notBlock = (block==1 ? 0 : 1);
	return ioctlsocket(fd, FIONBIO, (u_long *)&notBlock);
#else
	int flag = fcntl(fd, F_GETFL);
	if (block==1) flag &= ~O_NONBLOCK;//×èÈû
